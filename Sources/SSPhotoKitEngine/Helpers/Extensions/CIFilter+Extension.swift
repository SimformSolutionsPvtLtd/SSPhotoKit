...
extension CIFilter {
    ...
    public func getRange<V>(for key: String) -> ClosedRange<V> where V: BinaryFloatingPoint {
        guard  let attr = attributes[key] as? [String: Any],
               let upperBound = attr[kCIAttributeSliderMin] as? Double,
               let lowerBound = attr[kCIAttributeSliderMax] as? Double else {
            return 0...1
        }
    ...
    public func getDefaultValue<V>(for key: String) -> V where V: BinaryFloatingPoint {
        guard let attr = attributes[key] as? [String: Any],
              let value = attr[kCIAttributeDefault] as? Double else {
            return 0
        }
        return V(value)
    }
